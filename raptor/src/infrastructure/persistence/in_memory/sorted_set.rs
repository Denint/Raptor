use crate::{
    domain::{
        entities::stored_value::StoredValue, errors::DomainError,
        repositories::SortedSetRepository, value_objects::key::Key, value_objects::value::Value,
    },
...
impl SortedSetRepository for InMemoryStorage {
    async fn zadd(&self, key: &Key, score: f64, member: Vec<u8>) -> Result<bool, DomainError> {
        let is_new = {
            let mut entry = self
                .store
                .entry(key.clone())
                .or_insert_with(|| StoredValue::new(Value::SortedSet(Vec::new())));
    ...
    async fn zscore(&self, key: &Key, member: &[u8]) -> Result<Option<f64>, DomainError> {
        if let Some(mut stored_value) = self.store.get_mut(key) {
            ...
            stored_value.update_access();
            match &stored_value.value {
                Value::SortedSet(sorted_set) => Ok(sorted_set
                    .iter()
                    .find(|(_, m)| m == member)
                    .map(|(score, _)| *score)),
                _ => Err(DomainError::WrongType),
