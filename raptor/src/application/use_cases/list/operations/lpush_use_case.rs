...
pub struct LPushUseCase<R: ?Sized + ListRepository> {
    repository: Arc<R>,
}

impl<R: ?Sized + ListRepository> LPushUseCase<R> {
    pub fn new(repository: Arc<R>) -> Self {
        Self { repository }
    }

    #[inline]
    pub async fn execute(&self, input: LPushInput) -> Result<usize, DomainError> {
        let span = info_span!("audit.lpush_use_case", key = %input.key.as_str(), values_len = input.values.len());
        let _enter = span.enter();
        let result = self.repository.lpush(&input.key, input.values).await;
        event!(Level::INFO, op = "lpush", key = %input.key.as_str(), success = result.is_ok());
        if let Err(e) = &result {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_lpush_input_new() {
        let key = Key::new("test_key".to_string()).unwrap();
        let values = vec![b"value1".to_vec(), b"value2".to_vec()];
        let input = LPushInput::new(key.clone(), values.clone());
        assert_eq!(input.key, key);
        assert_eq!(input.values, values);
    }

    #[tokio::test]
    async fn test_lpush_use_case_new() {
        let mock_repo = Arc::new(MockListRepository::new());
        let _use_case = LPushUseCase::new(mock_repo);
    }

    #[tokio::test]
    async fn test_lpush_use_case_execute_success() {
        ...
        let use_case = LPushUseCase::new(Arc::new(mock_repo));
    ...
    #[tokio::test]
    async fn test_lpush_use_case_execute_single_value() {
        let mut mock_repo = MockListRepository::new();
    ...
    #[tokio::test]
    async fn test_lpush_use_case_execute_multiple_values() {
        let mut mock_repo = MockListRepository::new();
    ...
    #[tokio::test]
    async fn test_lpush_use_case_execute_empty_values() {
        let mut mock_repo = MockListRepository::new();
    ...
    #[tokio::test]
    async fn test_lpush_use_case_execute_error() {
        let mut mock_repo = MockListRepository::new();
