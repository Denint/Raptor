...
pub struct SRemUseCase<R: ?Sized + SetRepository> {
    repository: Arc<R>,
}

impl<R: ?Sized + SetRepository> SRemUseCase<R> {
    pub fn new(repository: Arc<R>) -> Self {
        Self { repository }
    }

    #[inline]
    pub async fn execute(&self, input: SRemInput) -> Result<usize, DomainError> {
        let span = info_span!("audit.srem_use_case", key = %input.key.as_str(), members_len = input.members.len());
        let _enter = span.enter();
        let result = self.repository.srem(&input.key, input.members).await;
        event!(Level::INFO, op = "srem", key = %input.key.as_str(), success = result.is_ok());
        if let Err(e) = &result {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_srem_input_new() {
        let key = Key::new("test_key".to_string()).unwrap();
        let members = vec![b"member1".to_vec(), b"member2".to_vec()];
        let input = SRemInput::new(key.clone(), members.clone());
        assert_eq!(input.key, key);
        assert_eq!(input.members, members);
    }

    #[tokio::test]
    async fn test_srem_use_case_new() {
        let mock_repo = Arc::new(MockSetRepository::new());
        let _use_case = SRemUseCase::new(mock_repo);
    }

    #[tokio::test]
    async fn test_srem_use_case_execute_success() {
        ...
        let use_case = SRemUseCase::new(Arc::new(mock_repo));
    ...
    #[tokio::test]
    async fn test_srem_use_case_execute_single_member() {
        let mut mock_repo = MockSetRepository::new();
    ...
    #[tokio::test]
    async fn test_srem_use_case_execute_partial_removal() {
        let mut mock_repo = MockSetRepository::new();
    ...
    #[tokio::test]
    async fn test_srem_use_case_execute_no_removal() {
        let mut mock_repo = MockSetRepository::new();
    ...
    #[tokio::test]
    async fn test_srem_use_case_execute_empty_members() {
        let mut mock_repo = MockSetRepository::new();
    ...
    #[tokio::test]
    async fn test_srem_use_case_execute_error() {
        let mut mock_repo = MockSetRepository::new();
